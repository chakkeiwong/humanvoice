...
brief, frozen concept baseline, and verification record cleared the path.
Parsers, linters, models, and build tools support those steps; none of them owns
the document.
...
\item[Humanization brief] The intended reader, genre, known vocabulary, named
exemplars, evidence boundary, privacy class, remote-inference authorization,
protected-object policy, voice constraints, and explicit unknowns, bound to one
immutable source.
\item[Concept baseline] The complete source-span partition, substantive concepts
and their teaching roles, explanation obligations, dependency order, scaffolding
dispositions, bidirectional coverage reviews, and the named freeze decision
recorded before any rewriting.
...
\item[Semantic preflight result] The build, integrity, concept-correspondence,
obligation-fulfillment, dependency-order, unsupported-addition, scaffolding,
writing, mutation, abstention, and repair results for a candidate revision.
\item[Release decision] The gate results, packet hashes, blocking reasons, and
named owner of release.
...
The relationships are more important than the field names. A brief and an
immutable snapshot own a frozen baseline; the baseline owns concepts,
obligations, and the rewrite units assigned to them; a finding belongs to a
source snapshot and a verification run; a revision answers one or more findings;
a protected-object comparison links parent and child snapshots; and a reader
...
until it is accepted as a patch against an approved rewrite unit; a candidate is
...
Humanization brief & Reader, genre, voice, disclosure, evidence boundary &
Defines the task and the material the writer may use \\
Baseline hash & Concepts, obligations, dependencies, unit assignments & Makes the
preserved meaning and writer context reproducible \\
...
The production entry point is \texttt{hv humanize}, which orchestrates the
resumable phases introduced in Chapter~\ref{ch:product} and specified in
Chapter~\ref{ch:proposal}:

\begin{description}[style=nextline,leftmargin=30mm,labelindent=0mm]
\item[\texttt{hv init}] Snapshots the immutable source and validates the
humanization brief, evidence boundary, privacy class, and protected-object
declarations. Critical blanks block the writer.
\item[\texttt{hv inventory}] Partitions every source span, extracts concepts and
teaching roles, runs the independent reverse coverage review, classifies
scaffolding, and freezes the reviewed baseline. An uncovered span or unresolved
classification blocks rewriting.
\item[\texttt{hv plan}] Assigns concepts, obligations, and dependencies to
rewrite units in prerequisite order and splits units on semantic boundaries.
\item[\texttt{hv rewrite}] Invokes the replaceable writer for one approved source
passage and records the context, concepts, and correspondence it returned.
\item[\texttt{hv preflight}] Runs build, integrity, correspondence, obligation,
dependency, addition, scaffolding, writing, and mutation checks against the
candidate revision. Unknown results remain blockers.
\item[\texttt{hv repair}] Applies a named causal repair, rebuilds, reruns
verification, and stops on a cycle limit or oscillation.
\item[\texttt{hv assemble}] Applies accepted patches to a copy of the source tree
by stable offsets, verifies untouched bytes, and builds the revision and its
...
Greenfield authoring from a brief remains reachable only under an explicit
\texttt{hv legacy} namespace. It is experimental, and its records cannot satisfy
a release gate.
...
The operational envelope is stated numerically in
Appendix~\ref{app:implementation-contract}: caps on source-tree size, rendered
pages, protected objects, repair cycles per unit, and model time on the reference
machine. Those are operating controls, not performance claims and never prose
targets. Exhausting one pauses or blocks work; it never authorizes deleting
content or releasing an incomplete revision. Measured runs either revise the caps
openly or narrow the supported document class.

\section{The result of this architecture}
\label{sec:architecture-conclusion}

Follow one manuscript through the system and the promise becomes testable. The
brief identifies the reader; the frozen baseline states every concept that must
survive and what each owes the reader; each rewritten passage records its
correspondence; preflight exposes a loss before release; a revision records a
human choice; a protected comparison exposes a meaning-bearing change; and a
clean reader view supplies the outcome. Engineers can replace a component as long
as it respects those boundaries.

The architecture does not guarantee a persuasive document. It makes the
failure legible and makes a comparative experiment possible. The next Part
specifies the

...
\section*{What the rescue shows about the product}
...
That manuscript is a test case, not the reason the product exists. Humanvoice
humanizes finished technical manuscripts. It sits beside the author's editor and
repository, not in place of them. Given an immutable source and an explicit
authorization, it inventories every substantive concept, freezes a
human-reviewed baseline, rewrites the source passage by passage, and runs a
fail-closed check before any version becomes a reader packet. A broad complaint
such as ``this is hard to read'' becomes a small set of located questions; the
objects that carry technical meaning stay protected; the author's decision stays
visible. It never certifies that prose is ``human,'' infers authorship from one
document, edits the source in place, shortens a manuscript to hit a length, or
asks an expensive reader to serve as its ordinary debugger.
...
The build proceeds in gated stages rather than one undifferentiated effort: the
typed source and concept model, the frozen concept baseline, source-grounded
rewriting, independent verification and repair, patch assembly and release, and
finally product evidence. Each stage has an explicit gate, and no gate may infer
a later evidence state from an earlier one. Specification, implementation,
automated verification, independent reproduction, and human reader evidence are
reported separately and never substituted for one another.

The sponsor should be able to answer three questions:
...
Build the humanization system to production quality against the gates in this
volume. Release externally only on reader evidence: a complete benchmark
manuscript, a held-out manuscript the rules were not tuned on, independent
reproduction, and named readers who can recover the meaning. Protected-object
counts, source-length ratios, and clean builds are necessary and not sufficient.
\end{decisionbox}
